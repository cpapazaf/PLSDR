...
class PLSDR(QMainWindow, Ui_MainWindow):
    ...
    """
        ...
        self.mode_control = MyCombo.Combo(
            self,
            self.config,
            self.mode_combo,
            self.change_modes,
        ...
        self.agc_control = MyCombo.Combo(
            self,
            self.config,
            self.agc_combo,
            self.set_agc_mode,
        ...
        self.bw_buttonlist = [
            self.if_bw_wide_button,
            self.if_bw_medium_button,
            self.if_bw_narrow_button]

        self.BW_WIDE = 0
        self.BW_MEDIUM = 1
        self.BW_NARROW = 2

        self.if_bw_buttongroup_control = MyButtonGroup.ButtonGroup(
            self.config,
            self.if_bw_button_group,
            self.set_bw_mode,
            'bw_mode',
            self.bw_buttonlist)

        self.gain_control_a = MySlider.Slider(
            self.config, self.gain_slider_a, self.set_named_gain, 'gain_a', 0, 50)
        self.gain_control_b = MySlider.Slider(
            self.config, self.gain_slider_b, self.set_named_gain, 'gain_b', 0, 50)
        self.gain_control_c = MySlider.Slider(
            self.config, self.gain_slider_c, self.set_named_gain, 'gain_c', 0, 50)
        self.gain_control_d = MySlider.Slider(
            self.config, self.gain_slider_d, self.set_named_gain, 'gain_d', 0, 50)

        self.af_gain_control = MySlider.Slider(
            self.config, self.af_gain_slider, self.set_af_gain, 'af_gain', 0, 1, 400)
        self.average_control = MySlider.Slider(
            self.config, self.averaging_slider, self.set_average, 'average', 1, .01)
        self.squelch_control = MySlider.Slider(
            self.config, self.squelch_slider, self.set_squelch, 'squelch_level', -130, 50, 400)
        ...
        self.bandwidth_control = MyCombo.Combo(
            self,
            self.config,
        ...
        self.fft_size_control = MyCombo.Combo(
            self,
            self.config,
            self.fft_size_combo,
        ...
        self.framerates = [
            ...
            n for n in (
                ...
                2,
                1)]
        self.framerate_control = MyCombo.Combo(
            self,
            self.config,
            self.framerate_combo,
            self.critical_change,
            'framerate',
            self.framerates)

        self.antenna_control = MyCombo.Combo(
            self,
            self.config,
            self.antenna_combo,
            self.change_antennas,
            'antenna')

        self.sample_rate_control = MyCombo.Combo(
            self,
            self.config,
        ...
        self.audio_rate_control = MyTextEntry.TextEntry(
            self, self.audio_rate_text, self.critical_change, 'audio_rate', 1e3, 60e3)

        self.cw_base_control = MyTextEntry.TextEntry(
            self, self.cw_base_text, self.critical_change, 'cw_base', 1e2, 3e3)

        self.audio_device_control = MyTextEntry.TextEntry(
            self, self.audio_device_text, self.critical_change, 'audio_device', 0, 0, True)

        self.corr_ppm_control = MyTextEntry.TextEntry(
            self, self.corr_ppm_text, self.set_corr_ppm, 'corr_ppm', -100, 100)

        self.corr_ppm_upc_control = MyTextEntry.TextEntry(
            self, self.corr_ppm_upc_text, self.set_corr_ppm_upc, 'corr_ppm_upc', -100, 100)

        self.upconvert_state_control = MyCheckbox.Checkbox(
            self, self.upconversion_checkbox, self.use_upconversion, 'upconvert_state')

        self.upconvert_trans_control = MyTextEntry.TextEntry(
            self,
            self.upconvert_trans_text,
            self.update_freq_event,
            'upconvert_trans',
            0,
            50e6)

        self.upconvert_freq_control = MyTextEntry.TextEntry(
            self,
            self.upconvert_freq_text,
            self.update_freq_event,
            'upconvert_freq',
            0,
            1000e6)

        self.offset_state_control = MyCheckbox.Checkbox(
            self, self.offset_checkbox, self.use_offset, 'offset_state')

        self.offset_freq_control = MyTextEntry.TextEntry(
            self, self.offset_freq_text, self.update_offset_freq, 'offset_freq', -10000, 10000)

        self.dc_offset_control = MyCheckbox.Checkbox(
            self, self.dc_offset_checkbox, self.set_dc_offset, 'dc_offset')

        self.iq_balance_control = MyCheckbox.Checkbox(
            self, self.iq_balance_checkbox, self.set_iq_balance, 'iq_balance')
    ...
    def first_read_config(self):
        self.setup_help()
        self.read_config()
        self.waterfall_widget = Waterfall.WaterfallWidget(
            self, self.config, self.waterfall_layout)
        self.fft_widget = FFTDisp.FFTDispWidget(
            self, self.config, self.fft_disp_layout)
        self.enabled = True
    ...
    def read_config(self):
        ...
        self.freq_table.verticalScrollBar().setValue(
            self.config['freqlist_scroll_position'])
        self.help_text_browser.verticalScrollBar().setValue(
            self.config['help_scroll_position'])
    ...
    def setup_freq_digits(self):
        ...
        self.freq_digits = []
        for n in range(10):
            self.freq_digits.append(
                FreqDigit.FreqDigit(
                    "0", self, self.dsp_digits[n]))
    ...
    def test_upconvert_mode(self):
        if self.upconvert_state_control is not None:
            if self.upconvert_state_control.get_value():
                if self.config['freq'] <= self.upconvert_trans_control.get_value():
                    return True
        return False
    ...
    def update_freq(self, f=None):
        if self.enabled:
            ...
            mf = self.config['freq'] + upconvert_offset + \
                self.radio.compute_offset_f()
            if self.radio.osmosdr_source is not None:
                #print("assigned freq: %f = %d" % (mf,int(mf)))
                self.radio.osmosdr_source.set_center_freq(int(mf), 0)
                self.radio.osmosdr_source.set_freq_corr(
                    self.acquire_corr_ppm(), 0)
    ...
    def populate_freq_list(self):
        ...
        if path is None:
            data = self.read_file(
                'frequency_spreadsheet/frequency_list.csv', 'rb')
        ...
        for mult in mults:
            for n, field in enumerate(self.fieldNames):
                if re.search("(?i).*%s.*" % mult, field) is not None:
                    rmult = mults[mult]
                    self.freq_field = n
                if re.search("(?i).*mode.*", field) is not None:
                    self.mode_field = n
        if self.freq_field == -1 or self.mode_field == -1:
            self.message_dialog(
                "Malformed Frequency List",
                "The present frequency-table source document (%s) is not correctly formatted -- please read the documentation." %
                path)
        else:
    ...
    def configure_device_combo(self):
        ...
        self.device_control = MyCombo.Combo(
            self,
            self.config,
            self.device_combo,
            self.select_device,
    ...
    def set_agc_mode(self, mode=None, name=None):
        ...
        if self.radio.osmosdr_source is not None:
            ...
            if self.radio.analog_agc_cc is not None:
                #print("setting AGC mode: %d" % mode)
                for inst in (
                        self.radio.analog_agc_cc,
                        self.radio.analog_agc_ff):
